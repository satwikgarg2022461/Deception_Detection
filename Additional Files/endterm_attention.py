...
class DiplomacyGraphModel(nn.Module):
    def __init__(self, bert_model_name=BERT_MODEL_NAME, lstm_hidden_dim=64, dropout_rate=0.1, num_classes=2):
        ...
        scores_dim = NUM_PLAYERS  # 7
        relationship_feature_dim = 1
        timestamp_feature_dim = 1

        # Define projection dimension for self-attention
        projection_dim = 512

        # Projection layers for each feature type
        self.text_proj = nn.Linear(bert_output_dim, projection_dim)
        self.scores_proj = nn.Linear(scores_dim, projection_dim)
        self.strength_proj = nn.Linear(relationship_feature_dim, projection_dim)
        self.timestamp_proj = nn.Linear(timestamp_feature_dim, projection_dim)

        # Self-attention layer
        self.attention = nn.MultiheadAttention(embed_dim=projection_dim, num_heads=4)

        # Dropout and classifier
        self.dropout = nn.Dropout(dropout_rate)
        self.classifier = nn.Linear(projection_dim, num_classes)  # Input dim is projection_dim after pooling

    def forward(self, input_ids, attention_mask, sequences, sender_names, receiver_names, timestamps):
        # Get BERT text embedding
        bert_outputs = self.bert(input_ids=input_ids, attention_mask=attention_mask)
        text_embedding = bert_outputs.pooler_output  # [batch_size, 768]

        # Get relationship strengths from LSTM
        all_relationship_strengths = self.relationship_lstm(sequences)  # [batch_size, NUM_EDGES]

        # Extract sender-receiver specific strengths
        ...
        sender_receiver_strengths_tensor = torch.stack(sender_receiver_strengths).unsqueeze(1)  # [batch_size, 1]

        # Extract scores from sequences
        scores = sequences[:, 0, :NUM_PLAYERS]  # [batch_size, 7]

        # Project each feature to projection_dim
        text_proj = self.text_proj(text_embedding)  # [batch_size, 128]
        scores_proj = self.scores_proj(scores)  # [batch_size, 128]
        strength_proj = self.strength_proj(sender_receiver_strengths_tensor)  # [batch_size, 128]
        timestamp_proj = self.timestamp_proj(timestamps)  # [batch_size, 128]

        # Stack features into a sequence: [batch_size, 4, 128]
        features = torch.stack([text_proj, scores_proj, strength_proj, timestamp_proj], dim=1)

        # Transpose for MultiheadAttention: [seq_len=4, batch_size, 128]
        features = features.transpose(0, 1)

        # Apply self-attention
        attn_output, _ = self.attention(features, features, features)  # [4, batch_size, 128]

        # Transpose back: [batch_size, 4, 128]
        attn_output = attn_output.transpose(0, 1)

        # Mean pool across sequence dimension: [batch_size, 128]
...
def evaluate_model(model, data_loader, loss_fn_sender, loss_fn_receiver, device, task='sender'):
    ...
    if all_preds.size > 0 and all_labels.size > 0:
        ...
        macro_f1, lie_f1 = 0.0, 0.0

    return avg_loss, macro_f1, lie_f1, all_labels, all_preds
...
def train_and_evaluate(task_name, train_loader, val_loader, test_loader, device):
    ...
    for epoch in range(EPOCHS):
        ...
        val_loss, val_macro_f1, val_lie_f1, _, _ = evaluate_model(model, val_loader, sender_loss_fn, receiver_loss_fn,
                                                                  device, task=task_name)
        ...
        if val_macro_f1 > best_val_macro_f1:
            ...
            test_loss, test_macro_f1, test_lie_f1, test_labels, test_preds = evaluate_model(
                model, test_loader, sender_loss_fn, receiver_loss_fn, device, task=task_name
            )
            ...
            if test_labels.size > 0:
                ...
                print(f"Confusion Matrix ({task_name.upper()}):\n{cm}")
                print(classification_report(test_labels, test_preds, target_names=['Truth', 'Lie'], zero_division=0))
